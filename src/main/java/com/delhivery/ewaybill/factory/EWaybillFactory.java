package com.delhivery.ewaybill.factory;
...
import com.delhivery.ewaybill.auth.GovtAuthentication;
import com.delhivery.ewaybill.common.constants.EWaybillConstant;
import com.delhivery.ewaybill.common.enums.ErrorCode;
...
public class EWaybillFactory {
    ...
    private static String generateEWaybill() {
        ...
        tempPayload.put("docNo", DateTimeUtility.getModularCurrentTimeStamp());

        Map<String, Object> payload = new HashMap<>();
        payload.put("action", "GENEWAYBILL");
        System.out.println("Dek before calling: " + auth.get("dec_sek").toString());
        payload.put("data", EncryptionService.aesEncrypt((auth.get("dec_sek").toString()), Utilities.getJsonString(tempPayload)));
        String jsonPayload = Utilities.getJsonString(payload);
        ...
        Response response = RestResource.post("/ewaybillapi/v1.03/ewayapi/", httpTemplateObject);
    ...
    public static void main(String[] args) {
      List<String> wbn =   getEWaybill(100);
        System.out.println(wbn);
    }
}
