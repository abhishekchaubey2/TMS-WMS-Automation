...
public class UnsetReturnDdIdForB2CAndDoc extends BaseTest {
    ...
    public UnsetReturnDdIdForB2CAndDoc(String factoryScenario, Integer packageCount) {
    ...
    public static Object[][] Different_type_pkg() {
        return new Object[][]{
                {"Scenario::When product type is B2C and package count 0", "B2C", 0},
                {"Scenario::When product type is B2C MPS and package count 4", "B2C MPS", 4},
                {"Scenario::When product type is B2C MPS and package count 10", "B2C MPS", 10},
                {"Scenario::When product type is B2C MPS and package count 11", "B2C MPS", 11},
                {"Scenario::When product type is DOC and package count 0", "DOC", 0},
                {"Scenario::When product type is DOC MPS and package count 4", "DOC MPS", 4},
                {"Scenario::When product type is DOC MPS and package count 10", "DOC MPS", 10},
                {"Scenario::When product type is DOC MPS and package count 11", "DOC MPS", 11},
        };
    }
    ...
    private static Object[] getDifferentTypePkgAndPackageCount(String scenario, String pdt, Integer packageCount) {
        return new Object[]{new UnsetReturnDdIdForB2CAndDoc(scenario, packageCount)};
    }

    @Test(dataProvider = "Different_state_type_pkg_for_dd_unset", dataProviderClass = manifestationData.class)
    private void testUnsetReturnDdIdForB2CAndDoc(String scenario, String state) {
        if (unsetReturnDdIdUtility.shouldProcess(scenario,factoryScenario)) {

            manifestData.set(TestModuleHelper.prepareManifestData(null, scenario.contains("DOC") ? "DOC" : "B2C", null, null).get());
